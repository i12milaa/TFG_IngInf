...
La Figura~\ref{fig:sistema_overview} muestra el funcionamiento general del sistema. En el lado izquierdo, los nodos físicos N1, N2 y N3 barren el perímetro vigilado emitiendo pulsos acústicos; cuando uno de ellos detecta un objeto, mide la distancia a ese obstáculo y envía el par \textit{(distancia, ángulo)} al servidor. El intercambio de información se produce en dos sentidos: el servidor envía a cada nodo la orden de disparo con el instante temporal y el ángulo de barrido asignados, garantizando la ausencia de interferencias acústicas mediante el esquema TDMA; el nodo devuelve la medida obtenida en ese ciclo. En el centro de control, el árbitro TDMA determina qué nodos son geométricamente incompatibles y, al detectar un conflicto, transfiere la información al módulo de estimación de posición, que combina las medidas de los nodos implicados para calcular las coordenadas del obstáculo. El resultado, junto con el estado global de la red, se transmite a la interfaz gráfica para su visualización en tiempo real.

%\begin{figure}[H]
%\centering
%\includegraphics[width=0.95\textwidth]{figura_sistema.png}
%\caption{Esquema general del funcionamiento del sistema de vigilancia distribuido.}
%\label{fig:sistema_overview}
%\end{figure}
